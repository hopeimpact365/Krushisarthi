...
app.get('/', (req, res) => {
  ...
  const envChecks = [
    ...
    { name: 'EASEBUZZ_KEY', check: checkEnvVar('EASEBUZZ_KEY', (v) => ({ valid: v.length > 3, reason: 'Too short' })) },
    { name: 'EASEBUZZ_SALT', check: checkEnvVar('EASEBUZZ_SALT', (v) => ({ valid: v.length > 3, reason: 'Too short' })) },
    { name: 'EASEBUZZ_ENV', check: checkEnvVar('EASEBUZZ_ENV', (v) => ({ valid: ['test', 'prod'].includes(v), reason: 'Must be test or prod' })) },
