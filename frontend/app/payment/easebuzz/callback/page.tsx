...
function CallbackContent() {
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0d] text-stone-200 flex items-center justify-center p-6 font-sans">
      <div className="max-w-md w-full bg-[#141414] border border-stone-800 rounded-2xl p-8 shadow-2xl text-center space-y-6">
        {status === "verifying" && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-stone-400">
              Please wait while we confirm your payment transaction with Easebuzz...
            </p>
          </div>
        )}
        ...
        {status === "failed" && (
          <div className="space-y-6">
            ...
            <div className="pt-2 flex gap-4">
              <button
                type="button"
                onClick={() => router.push(`/payment/easebuzz?orderId=${orderId}`)}
                className="flex-1 bg-orange-500 text-stone-950 font-bold py-3 rounded-lg hover:bg-orange-400 transition-colors text-sm"
              >
                Retry Payment
              </button>
              <button
